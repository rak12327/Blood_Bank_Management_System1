        ...
        <Fragment>
            ...
            <div className='h-[100%] max-h-[28rem] overflow-y-auto'>
                {listData?.requestList.map((state, index) => (
                    <Accordion open={open === state._id} icon={<ChevronUp id={state._id} open={open} />} key={state._id}>
                        <AccordionHeader onClick={() => handleOpen(state._id)}
                            className="text-sm">
                            <p>
                                ...
                                <div className='flex items-center justify-between flex-col'>
                                    <div className='flex items-center justify-between'>
                                        <div >
                                            ...
                                            <input className="border-b-2 border-b-[black] text-center outline-none w-[50%]" defaultValue={state?.unit} />
                                        </div>
                                        <div >
                                            <label className="mr-[.5rem]">Price</label>
                                            <input className="border-b-2 border-b-[black] text-center outline-none w-[50%]" defaultValue={`$ ` + state?.price} />
                                        </div>
                                        <div >
                                            <label className="mr-[.5rem]">Collecting Data</label>
                                            <input className="border-b-2 border-b-[black] text-center outline-none w-[50%]" defaultValue={formatDate(state?.dateOfReceivingBlood)} />
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between flex-row w-[100%] py-[1rem]">
                                        <button className="w-[5rem] py-[.5rem] bg-[#000] text-[#fff] text-sm rounded">Delete</button>
