...
const RequestForm = () => {
  ...
  const [input, setInput] = useState({ patientName: "", age: "", gender: "", hospitalName: "", hospitalPhoneNumber: "", consultantName: "", dateOfReceivingBlood: "" })
  ...
  const submitHandler = async (e) => {
    ...
    if (userData.name && input.age && userData.adhaarNumber &&
      input.consultantName && input.gender &&
      input.hospitalName && input.hospitalPhoneNumber &&
      input.patientName) {
      console.log(input);
      try {
        await dispatch(requestFormThunk({ input: { ...input, email: userData?.email, componentName: data.componentName, price: data.price, unit: data.unit }, dispatch, navigate, token })).unwrap()
      } catch (rejectedValueOrSerializedError) {
        dispatch(openAlert({ message: rejectedValueOrSerializedError?.message, color: "red" }))
      }
      setTouch({ patientName: false, age: false, gender: false, hospitalName: false, hospitalPhoneNumber: false, consultantName: false, dateOfReceivingBlood: false })
      setInput({ patientName: "", age: "", gender: "", hospitalName: "", hospitalPhoneNumber: "", consultantName: "", dateOfReceivingBlood: null })
    } else {
      setTouch({ patientName: true, age: true, gender: true, hospitalName: true, hospitalPhoneNumber: true, consultantName: true, dateOfReceivingBlood: true })
